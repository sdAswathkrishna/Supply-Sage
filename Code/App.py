...
with head:
        ...
        str_path = "D:\\Supply Sage\\Images\\logo.png"
...
if sel=='Upload Data':
    ...
    if ax1:
        str_path = "D:\\Supply Sage\\Images\\cust.png"
        img = pl.open(str_path)
    ...
    if ax2:
        str_path = "D:\\Supply Sage\\Images\\prod.png"
        img = pl.open(str_path)
        st.image(img, width=350)
    ...
    if ax3:
        str_path = "D:\\Supply Sage\\Images\\targ.png"
        img = pl.open(str_path)
        st.image(img, width=350)
    ...
    if ax4:
        str_path = "D:\\Supply Sage\\Images\\ord.png"
        img = pl.open(str_path)
...
if sel=="Supply Chain Analysis":
    ...
    op=st.selectbox("Choose an Analysis",['Verifying Datasets','Key Performance Indicators','In-Full Percentage Over Time','On-Time Percentage Over Time','On-Time In-Full Percentage Over Time'])
    if(op=='Verifying Datasets'):

        st.markdown("""Verifying Datasets""")
        # Try to access df1,df2,df3,df4 from the session state
        if ('df1' in st.session_state) & ('df2' in st.session_state) & ('df3' in st.session_state) & ('df4' in st.session_state):
            sca.run(st.session_state.df1,st.session_state.df2,st.session_state.df3,st.session_state.df4)
        else:
            st.warning(warn)

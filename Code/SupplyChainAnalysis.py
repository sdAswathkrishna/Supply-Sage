...
def run( df1, df2, df3, df4):
    ax19 = st.checkbox("Show Description",key="19")
    if ax19:    
        
        st.markdown("""Display the initial few rows (head) of each dataframe in the Streamlit app to give a preview of the datasets.

*Parameters:*
df1: Customer dataset dataframe.
df2: Product dataset dataframe.
df3: Target of orders dataset dataframe.
df4: Order dataset dataframe.""",True)
    st.write(df1.head())
    st.write(df2.head())
    st.write(df3.head())
    st.write(df4.head())
